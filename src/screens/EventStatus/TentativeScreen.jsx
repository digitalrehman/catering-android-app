...
const TentativeScreen = () => {
  ...
  useEffect(() => {
    fetchTentativeEvents();
  }, []);

  // ✅ Fetch event details for PDF generation
  ...
  const fetchTentativeEvents = async () => {
    try {
      ...
      console.error('Error fetching tentative events:', error);
  ...
  return (
    <LinearGradient colors={COLORS.GRADIENT_PRIMARY} style={{ flex: 1 }}>
      ...
      />
      <AppHeader title="Tentative Events" />
...
export default TentativeScreen;
